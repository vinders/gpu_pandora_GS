...
void ColorTheme::updateTheme(video_api::Renderer& renderer, ColorThemeType type) {
  switch (type) {
    case ColorThemeType::darkBlue: { // TODO -> teinte -2 sur chaque couleur
      ...
      SET_THEME_COLOR(renderer,  13, 39, 59, 1.f, backgroundCorner)
    ...
    case ColorThemeType::darkYellow: {
      ...
      SET_THEME_COLOR(renderer,  18, 14,  5, 1.f, background)
      SET_THEME_COLOR(renderer,  52, 43, 21, 1.f, backgroundCorner) // 76, 56, 19
      SET_THEME_COLOR(renderer,  65, 46, 14, 1.f, scrollbarControl)
      SET_THEME_COLOR(renderer, 114, 79, 27, 1.f, scrollbarThumb)
      SET_THEME_COLOR(renderer,  92, 68, 22, 0.75f, lineSelectorControl)
      SET_THEME_COLOR(renderer,  71, 55, 28, 1.f, tooltipControl)
