...
void MessageResources::fillMessagesEN() {
  ...
  GET_UI_MESSAGE(generalSettings, GeneralSettingsMessages::autoloadOnStart) = U"Autoload on start";
  GET_UI_MESSAGE(generalSettings, GeneralSettingsMessages::autoloadOnStart_tooltip) = U"Automatically load the latest save from the active slot when the emulator is started";
  ...
  GET_UI_MESSAGE(screenStretching, ScreenStretchingMessages::showOverscanArea_tooltip) = U"Extra pixels out of screen boundaries: horizontal (Capcom games, Pandemonium...) + vertical (most NTSC games)";
  ...
  GET_UI_MESSAGE(screenStretching, ScreenStretchingMessages::blackBorders) = U"Add black borders (X/Y)";
  GET_UI_MESSAGE(screenStretching, ScreenStretchingMessages::blackBorders_tooltip) = U"Prevent border flickering, by adding black borders (values: horizontal/vertical)";
  ...
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::title) = U"Smoothing/upscaling";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::frameBufferGroup) = U"Framebuffer settings";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::internalResolution) = U"Internal resolution (X/Y)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::internalResolution_tooltip) = U"Framebuffer resolution multiplier (above 4 (X) and 6 (Y): very slow and not supported by all GPUs)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::internalResolution_prefix) = U"Drawing area:  ";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::internalResolution_to) = U" to ";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::internalResolution_suffix) = U" (game specific)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::mdecMovieFilter) = U"MDEC movie filter";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::mdecMovieFilter_tooltip) = U"YUV filtering to decode full motion videos (MDEC unit emulation)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::disabled) = U"Disabled";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::grain_photo) = U"Photographic";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::grain_gauss) = U"Gaussian noise";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::screenGroup) = U"Display settings";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::screenUpscaling) = U"Screen upscaling";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::screenUpscaling_tooltip) = U"Advanced/sharp frame scaling to improve sharpness, especially with 2D and pixel art (can be very slow)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::displaySize_prefix) = U"Display size:  ";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::screenBlur) = U"Screen blur";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::screenBlur_tooltip) = U"Blur the whole frame, to reduce noise/sharpness/pixelation (move left to disable)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::screenGrain) = U"Screen grain";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::screenGrain_tooltip) = U"Add grain to the whole frame, for visual effects or to fix unnatural blurs";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::smoothing_nearest) = U"Nearest neighbor";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::smoothing_bilinear) = U"Bilinear (smoother)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::smoothing_bicubic) = U"Bicubic (enhanced, slower)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::smoothing_lanczos) = U"Lanczos (enhanced, slower)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::textureGroup) = U"Texture settings";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::textureUpscaling) = U"Texture upscaling";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::textureUpscaling_tooltip) = U"Advanced/sharp image scaling to improve texture quality, especially with 2D and pixel art (can be very slow)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::textureSmoothing) = U"Texture smoothing";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::textureSmoothing_tooltip) = U"Texture minification/magnification interpolation type (pixel transition smoothing)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::textureGrain) = U"Texture grain";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::textureGrain_tooltip) = U"Add grain to scaled textures, to fix unnatural smoothness/sharpness";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::textureSplatting) = U"Texture splatting";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::textureSplatting_tooltip) = U"Combine adjacent texture borders to smoothen transitions between them (move left to disable)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::spriteGroup) = U"2D-sprite settings";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::spriteTextureSettings) = U"Same as textures";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::spriteTextureSettings_tooltip) = U"Use the same upscaling/smoothing/grain settings for textures and sprites";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::spriteUpscaling) = U"Sprite upscaling";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::spriteUpscaling_tooltip) = U"Advanced/sharp image scaling to improve sprite quality, especially with 2D and pixel art (can be very slow)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::spriteSmoothing) = U"Sprite smoothing";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::spriteSmoothing_tooltip) = U"2D sprite minification/magnification interpolation type (pixel transition smoothing)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::spriteGrain) = U"Sprite grain";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::spriteGrain_tooltip) = U"Add grain to scaled 2D sprites, to fix unnatural smoothness/sharpness";
...
void MessageResources::fillMessagesFR() {
  ...
  GET_UI_MESSAGE(generalSettings, GeneralSettingsMessages::autosaveOnExit_tooltip) = U"Sauvegarder automatiquement l'état du jeu (dans l'emplacement actif) lorsque l'émulateur est fermé";
  GET_UI_MESSAGE(generalSettings, GeneralSettingsMessages::autoloadOnStart) = U"Chargement au démarrage";
  GET_UI_MESSAGE(generalSettings, GeneralSettingsMessages::autoloadOnStart_tooltip) = U"Charger automatiquement la dernière sauvegarde (emplacement actif) lorsque l'émulateur démarre";
  ...
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::frameBufferGroup) = U"Framebuffer settings";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::internalResolution) = U"Internal resolution (X/Y)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::internalResolution_tooltip) = U"Framebuffer resolution multiplier (above 4 (X) and 8 (Y): very slow and not supported by all GPUs)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::internalResolution_prefix) = U"Drawing area:  ";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::internalResolution_to) = U" to ";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::internalResolution_suffix) = U" (game specific)";
  GET_UI_MESSAGE(smoothingUpscaling, SmoothingUpscalingMessages::mdecMovieFilter) = U"MDEC movie filter";
...
void MessageResources::fillMessagesDE() {
  ...
  GET_UI_MESSAGE(generalSettings, GeneralSettingsMessages::autosaveOnExit_tooltip) = U"Automatisch Speicherung im aktiven Speicherplatz, wenn der Emulator geschlossen wird";
  GET_UI_MESSAGE(generalSettings, GeneralSettingsMessages::autoloadOnStart) = U"Laden beim Start";
  GET_UI_MESSAGE(generalSettings, GeneralSettingsMessages::autoloadOnStart_tooltip) = U"Automatisch Beladung aus dem aktiven Steckplatz, wenn der Emulator gestartet wird";
...
void MessageResources::fillMessagesES() {
  ...
  GET_UI_MESSAGE(generalSettings, GeneralSettingsMessages::autoloadOnStart) = U"Carga al inicio";
  GET_UI_MESSAGE(generalSettings, GeneralSettingsMessages::autoloadOnStart_tooltip) = U"Cargue automáticamente el último guardado desde la ranura activa cuando se inicie el emulador";
